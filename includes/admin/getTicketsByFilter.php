...
if(isset($_POST['filter_by_type']) && $_POST['filter_by_type']!='all'){
	$flagUseWhere=true;
	$where.="t.type='". sanitize_text_field($_POST['filter_by_type'])."' ";
}
/* BEGIN CLOUGH I.T. SOLUTIONS MODIFICATION
 * Update 2 - Create 'All Active' filter
 * Modify filter where clause
 */
if(isset($_POST['filter_by_status']) && $_POST['filter_by_status'] != 'all' && $_POST['filter_by_status'] != 'deleted') {
	$where .= ( $flagUseWhere ) ? 'AND ' : '';
	$flagUseWhere = true;
	$where .= "t.status='" . sanitize_text_field($_POST['filter_by_status']) . "' ";
}
else if(isset($_POST['filter_by_status']) && $_POST['filter_by_status'] == 'deleted'){
        $where .= ( $flagUseWhere ) ? 'AND ' : '';
	$flagUseWhere = true;
	$where .= "t.active='0' ";
...
if(isset($_POST['filter_by_category']) && $_POST['filter_by_category']!='all'){
	$where.=($flagUseWhere)?'AND ':'';
	$flagUseWhere=true;
	$where.="c.id='".intval(sanitize_text_field($_POST['filter_by_category']))."' ";
}
...
foreach ($customFieldsDropDown as $field){
	if(isset($_POST['cust'.$field->id]) && $_POST['cust'.$field->id]!='all'){
		$where.=($flagUseWhere)?'AND ':'';
		$flagUseWhere=true;
		$where.="t.cust".$field->id."='".sanitize_text_field($_POST['cust'.$field->id])."' ";
	}
}
...
if(isset($_POST['filter_by_priority']) &&  $_POST['filter_by_priority']!='all'){
	$where.=($flagUseWhere)?'AND ':'';
	$flagUseWhere=true;
	$where.="t.priority='".sanitize_text_field($_POST['filter_by_priority'])."' ";
}
switch($_POST['filter_by_selection'])
{
	case 'id':
		if($_POST['filter_by_search']!=''){
			...
			if($total_cust_field){
				foreach ($customFields as $field){
					$custCondition.="OR t.cust".$field->id." LIKE '%".sanitize_text_field($_POST['filter_by_search'])."%' ";
				}
			}
	
			$where.="t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.id=".sanitize_text_field($_POST['filter_by_search'])." OR t.id LIKE '%".sanitize_text_field($_POST['filter_by_search'])."%') ";
		}
		break;
	case 'text':
		if($_POST['filter_by_search']!=''){
			...
			$where.="t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.subject LIKE '%".sanitize_text_field($_POST['filter_by_search'])."%' OR th.body LIKE '%".sanitize_text_field($_POST['filter_by_search'])."%' ".$custCondition.") ";
		}
		break;
	case 'created_by':
		if($_POST['filter_by_search']!=''){
			...
			$term=esc_attr( sanitize_text_field($_POST['filter_by_search']) );
			...
			if(is_array($users) && count($users)>0)
			{
				...
				$where.="t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.guest_name LIKE '%".sanitize_text_field($_POST['filter_by_search'])."%' OR t.guest_email LIKE'%".sanitize_text_field($_POST['filter_by_search'])."%')) ";
			}
			else
			{
				$where.="t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.guest_name LIKE '%".sanitize_text_field($_POST['filter_by_search'])."%' OR t.guest_email LIKE'%".sanitize_text_field($_POST['filter_by_search'])."%') ";
...
$limit_start=intval(sanitize_text_field($_POST['page_no']))*intval(sanitize_text_field($_POST['filter_by_no_of_ticket']));
$limit="LIMIT ".$limit_start.",".intval(sanitize_text_field($_POST['filter_by_no_of_ticket']))." ";
$sql.=$limit;
