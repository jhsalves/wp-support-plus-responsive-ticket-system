...
<?php 
foreach ($wpsp_et_reply_ticket['templates'] as $key=>$val){
        echo '{'.$key.'} - '.__($val,'wp-support-plus-responsive-ticket-system').'<br>';
}
?>
</div>
<hr>
<form id='frmETReplyTicket'>
	<span class="label label-info wpsp_title_label">
	<?php _e("Reply Notification Email", 'wp-support-plus-responsive-ticket-system' );?>
	</span><br><br>
	<table class='wpsp_et_role_tbl'>
	  ...
	  <tr>
	    ...
	    <td><input type="radio" name='etEnableCustomer' value="1" <?php echo ($wpsp_et_reply_ticket['notify_to']['customer'])?'checked':'';?>></td>
	    <td><input type="radio" name='etEnableCustomer' value="0" <?php echo ($wpsp_et_reply_ticket['notify_to']['customer'])?'':'checked';?>></td>    
	  </tr>
	  <tr>
	  	<td>
	    ...
	    <td><input type="radio" name='etEnableStaffAdmin' value="1" <?php echo ($wpsp_et_reply_ticket['notify_to']['administrator'])?'checked':'';?>></td>
	    <td><input type="radio" name='etEnableStaffAdmin' value="0" <?php echo ($wpsp_et_reply_ticket['notify_to']['administrator'])?'':'checked';?>></td>    
	  </tr>
	  <tr>
	  	<td><?php _e("Supervisor", 'wp-support-plus-responsive-ticket-system' );?></td>
	    <td><input type="radio" name='etEnableStaffSupervisor' value="1" <?php echo ($wpsp_et_reply_ticket['notify_to']['supervisor'])?'checked':'';?>></td>
	    <td><input type="radio" name='etEnableStaffSupervisor' value="0" <?php echo ($wpsp_et_reply_ticket['notify_to']['supervisor'])?'':'checked';?>></td>    
	  </tr>
	  <tr>
	  	<td>
	    ...
	    <td><input type="radio" name='etEnableStaffAssignedAgent' value="1" <?php echo ($wpsp_et_reply_ticket['notify_to']['assigned_agent'])?'checked':'';?>></td>
	    <td><input type="radio" name='etEnableStaffAssignedAgent' value="0" <?php echo ($wpsp_et_reply_ticket['notify_to']['assigned_agent'])?'':'checked';?>></td>    
	  </tr>
	  <tr>
	  	<td><?php _e("All Agents", 'wp-support-plus-responsive-ticket-system' );?></td>
	    <td><input type="radio" name='etEnableStaffAllAgent' value="1" <?php echo ($wpsp_et_reply_ticket['notify_to']['all_agents'])?'checked':'';?>></td>
	    <td><input type="radio" name='etEnableStaffAllAgent' value="0" <?php echo ($wpsp_et_reply_ticket['notify_to']['all_agents'])?'':'checked';?>></td>    
	  </tr>
	</table><br>
	<b><?php _e("Subject", 'wp-support-plus-responsive-ticket-system' );?>:</b>
        [<?php _e($advancedSettings['ticket_label_alice'][1], 'wp-support-plus-responsive-ticket-system' );?> <?php echo $advancedSettings['wpsp_ticket_id_prefix'];?>] <input type="text" id='wpsp_et_staff_email_subject_for_reply' name='wpsp_et_staff_email_subject' style="width: 50%;" value='<?php echo htmlspecialchars(stripcslashes($wpsp_et_reply_ticket['reply_subject']), ENT_QUOTES);?>'/><br>
