...
<?php if($arrayVisible['closeticket'] && $isVisible && !$generalSettings['close_ticket_btn_status_val']=='' && $generalSettings['close_ticket_btn_status_val']!=$ticket->status && $FrontEndDisplaySettings['wpsp_hideCloseTicket']){
    ...
    ?>    
    <button  style="<?php echo $btnStyle;?>" id="wpsp_close_btn_action" class="btn btn-primary wpsp_ticket_nav_btn" onclick="wpsp_closeTicketStatus(<?php echo $ticket_id;?>,'<?php echo $generalSettings['close_ticket_btn_status_val'];?>','<?php echo wp_create_nonce($ticket_id);?>');" > <?php echo $generalSettings['close_btn_alice'];?></button>
<?php }?>
...
<?php do_action('wpsp_getindi_front_after_buttons_icon_planner',$ticket);?>
<div id="wpsp_show_more">
    <?php 
    do_action('wpsp_getindivisual_frontend_actions_prepend_inside_more',$ticket);
...
<h3>
    ...
    <?php if($current_user->has_cap('manage_support_plus_ticket')){ echo $editCustomField? '<img alt="Edit" class="wpsp_edit_fields" title="Edit" onclick="getEditCustomField('. $ticket_id .')" src="'.WCE_PLUGIN_URL.'asset/images/edit.png'.'" />':''; }?>
</h3>
<!-- Custom Field -->
...
<script type="text/javascript">
    ...
    function replyonclick(cid){
        if( display_ticket_data.ckeditor_enable_for_loggedin=='1'){
            ...
            var x=document.getElementById("reply"+cid);
            jQuery("#replyBody").val(value+x.innerText);
        }
