...
$filter_by_status_front       = sanitize_text_field($_POST['filter_by_status_front']);
$filter_by_category_front     = sanitize_text_field($_POST['filter_by_category_front']);

if($filter_by_category_front != 'all') {
	$filter_by_category_front = intval($filter_by_category_front);
}

$filter_by_selection_front    = sanitize_text_field($_POST['filter_by_selection_front']);
$filter_by_search_front       = sanitize_text_field($_POST['filter_by_search_front']);
if(!empty($_POST['filter_by_no_of_ticket_front'])){
    $filter_by_no_of_ticket_front = intval(sanitize_text_field($_POST['filter_by_no_of_ticket_front']));
}else{
    $filter_by_no_of_ticket_front=10;
}
...
foreach ($_POST as $key=>$val){
        if($key!='page_no' && $key!='action'){
            $user_filter[sanitize_text_field($key)] = sanitize_text_field($val);
        }
}
...
if(isset($filter_by_category_front) && $filter_by_category_front!='all'){
	$where.="AND c.id='".$filter_by_category_front."' ";
}
...
foreach ($customFieldsDropDown as $field){
	if(isset($_POST['cust'.$field->id]) && $_POST['cust'.$field->id]!='all'){
		$where.="AND t.cust".$field->id."='".sanitize_text_field($_POST['cust'.$field->id])."' ";
	}
}

if(isset($filter_by_selection_front)){
	switch($filter_by_selection_front)
	{
		case 'id':
			if($filter_by_search_front!=''){
				//custome fields
				$custCondition='';
				$customFields = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wpsp_custom_fields" );
				...
				if($total_cust_field){
					foreach ($customFields as $field){
						$custCondition.="OR t.cust".$field->id." LIKE '%".$filter_by_search_front."%' ";
					}
				}
	
				$where.="AND t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.id=".$filter_by_search_front." OR t.id LIKE '%".$filter_by_search_front."%') ";
			}
			break;
		case 'text':
			if($filter_by_search_front!=''){
				...
				$where.="AND t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.subject LIKE '%".$filter_by_search_front."%' OR th.body LIKE '%".$filter_by_search_front."%' ".$custCondition.") ";
			}
			break;
                case 'created_by':
 		    if($filter_by_search_front!=''){
 			$term=esc_attr( $filter_by_search_front );
 			...
 			{
 				$where.="AND t.id IN (SELECT DISTINCT t.id from {$wpdb->prefix}wpsp_ticket t INNER JOIN {$wpdb->prefix}wpsp_ticket_thread th ON t.id=th.ticket_id WHERE t.guest_name LIKE '%".$filter_by_search_front."%' OR t.guest_email LIKE'%".$filter_by_search_front."%') ";
 			}
 		    }
		    break;
	}
...
$limit_start=$page_no*$filter_by_no_of_ticket_front;
$limit="LIMIT ".$limit_start.",".$filter_by_no_of_ticket_front." ";
