...
if ( current_filter() != 'plugins_loaded' || $installed_version != WPSP_VERSION ) {
        ...
        /*
         * remove faulty indexes in tables in old versions
         */
        if(get_option( 'wpsp_remove_faulty_indexes' ) === false){
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='subject'");
            if($key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket DROP INDEX  `subject`");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='status'");
            if($key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket DROP INDEX  `status`");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='priority'");
            if($key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket DROP INDEX  `priority`");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='type'");
            if($key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket DROP INDEX  `type`");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_custom_status WHERE Key_name='name'");
            if($key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_custom_status DROP INDEX  `name`");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_custom_priority WHERE Key_name='name'");
            if($key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_custom_priority DROP INDEX  `name`");
            }
            
            update_option('wpsp_remove_faulty_indexes',TRUE);
        }
	...
	dbDelta( $sql );
        ...
        $coloums=$wpdb->get_results("SHOW COLUMNS FROM {$wpdb->prefix}wpsp_canned_reply like '%visibility'");         
        if(count($coloums)==0){             
            $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_canned_reply ADD visibility varchar(50) NULL DEFAULT 'private'");         
        }
	...
	update_option( 'wp_support_plus_version', WPSP_VERSION );
        ...
        /*
         * Add new indexes OR repair faulty indexes
         */
        if( get_option( 'wpsp_repair_faulty_indexes' ) === false ){
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='subject'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket ADD INDEX  `subject`(`subject`)");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='status'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket ADD INDEX  `status`(`status`)");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='priority'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket ADD INDEX  `priority`(`priority`)");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='type'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket ADD INDEX  `type`(`type`)");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_ticket WHERE Key_name='active'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_ticket ADD INDEX  `active`(`active`)");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_custom_status WHERE Key_name='name'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_custom_status ADD INDEX  `name`(`name`)");
            }
            
            $key=$wpdb->get_results("SHOW INDEX FROM {$wpdb->prefix}wpsp_custom_priority WHERE Key_name='name'");
            if(!$key){
                $wpdb->query("ALTER TABLE {$wpdb->prefix}wpsp_custom_priority ADD INDEX  `name`(`name`)");
            }
            
            update_option( 'wpsp_repair_faulty_indexes',TRUE );
        }
	...
	*/
	...
	if(!isset($FrontEndDisplaySettings['front_end_display_alice']))
	{
 	...
 	if(!isset($FrontEndDisplaySettings['front_end_display_alice'][13])){
        ...
        $generalSettings=get_option( 'wpsp_general_settings' );
        if(!isset($generalSettings['wpsp_display_deleted_tickets'])){
            $generalSettings['wpsp_display_deleted_tickets']=0;
            update_option('wpsp_general_settings',$generalSettings);
        }
        
        $FrontEndDisplaySettings=get_option( 'wpsp_front_end_display_settings' );
        if(!isset($FrontEndDisplaySettings['wpsp_restoreTicket'])){
            $FrontEndDisplaySettings['wpsp_restoreTicket']=1;
            update_option('wpsp_front_end_display_settings',$FrontEndDisplaySettings);
        }
	...
	if(!isset($FrontEndDisplaySettings['front_end_display_alice'][14]))
	{
        ...
        $FrontEndDisplaySettings=get_option('wpsp_front_end_display_settings' );
        if(!isset($FrontEndDisplaySettings['wpsp_rt_bc'])){
                $FrontEndDisplaySettings['wpsp_rt_bc']='#428bca';                
                update_option('wpsp_front_end_display_settings',$FrontEndDisplaySettings);
        }
        
        $FrontEndDisplaySettings=get_option('wpsp_front_end_display_settings' );
        if(!isset($FrontEndDisplaySettings['wpsp_rt_fc'])){
                $FrontEndDisplaySettings['wpsp_rt_fc']='#fff';                
                update_option('wpsp_front_end_display_settings',$FrontEndDisplaySettings);
        }
