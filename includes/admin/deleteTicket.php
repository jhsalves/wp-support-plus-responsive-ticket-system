...
if( $_POST['action'] != 'deleteTicketMultiple' ){
...
if($ticket->active==0){
    $sql="SELECT attachment_ids FROM {$wpdb->prefix}wpsp_ticket_thread WHERE ticket_id =".$ticket_id;
...
if($ticket->active==0){
    ...
    $wpdb->update($wpdb->prefix.'wpsp_ticket',array('active'=>0),array('id'=>$ticket_id));
}
...
if($to_new && apply_filters('wpsp_send_ticket_delete_mail',$send_delete_mail,$ticket)){
    $headers=apply_filters('wpsp_after_send_staff_email_in_deleteticket_template',$headers,$ticket,$piping_emails,$ignore_emails);
