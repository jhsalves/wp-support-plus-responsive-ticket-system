<h3><?php _e('Add New Canned Reply','wp-support-plus-responsive');?>
...
<form id="wpsp_add_canned" onsubmit="setCannedReply(this,event);" method="post"> 
    ...
    <select name="visibility">             
        <option value="private"><?php _e('Private','wp-support-plus-responsive');?></option>             
        <option value="public"><?php _e('Public','wp-support-plus-responsive');?></option>         
    </select>         
    <br><br>
    <input type="hidden" name="action" value="wpsp_setCannedReply"/>
    
    <button type="submit" class="btn btn-success"><?php _e('Submit','wp-support-plus-responsive');?></button> 
</form>