...
?>
<button class="btn btn-primary wpsp_ticket_nav_btn" id="btnChangeTicketStatus" onclick="getChangeTicketStatus(<?php echo $ticket_id; ?>);"><?php _e('Change Status', 'wp-support-plus-responsive-ticket-system'); ?></button>
<?php
}?>
...
<?php
     ...
     ?>
         <button class="btn btn-primary wpsp_ticket_nav_btn"onclick="assignAgent(<?php echo $ticket_id; ?>);"><?php _e('Assign Agent', 'wp-support-plus-responsive-ticket-system'); ?></button>
     <?php
     }
...
<?php if ($isVisible && $arrayVisible['closeticket'] && !$generalSettings['close_ticket_btn_status_val'] == '' && $generalSettings['close_ticket_btn_status_val'] != $ticket->status) { ?>
    <button id="wpsp_close_btn_action" class="btn btn-primary wpsp_ticket_nav_btn" onclick="wpsp_closeTicketStatus(<?php echo $ticket_id; ?>, '<?php echo $generalSettings['close_ticket_btn_status_val']; ?>', '<?php echo wp_create_nonce($ticket_id);?>');" > <?php echo $generalSettings['close_btn_alice']; ?></button>
<?php } ?>
...
<?php do_action('wpsp_getindi_after_buttons_icon_planner',$ticket);?>

<div id="wpsp_show_more">
    <?php 
    do_action('wpsp_getindivisual_backend_actions_prepend_inside_more',$ticket);
    
    if($isVisible && $arrayVisible['clone']){
    ?>
    <button class="btn btn-primary wpsp_ticket_nav_btn" id="clone" onclick="cloneTicket(<?php echo $ticket_id; ?>);"><?php _e('Clone Ticket', 'wp-support-plus-responsive-ticket-system'); ?></button>
    <?php
    }?> 
<?php
if (current_user_can('manage_options') && $arrayVisible['raisedby']) {
    ?><button class="btn btn-primary wpsp_ticket_nav_btn" onclick="getRaisedByTicketUser(<?php echo $ticket_id; ?>);"><?php _e('Change Raised By', 'wp-support-plus-responsive-ticket-system'); ?></button>&nbsp;<?php
    } else {
        foreach ($advancedSettings['modify_raised_by'] as $modifyRaisedBy) {
            if ($arrayVisible['raisedby'] && (($modifyRaisedBy == 'wp_support_plus_agent') && $current_user->has_cap('manage_support_plus_ticket')) || (($modifyRaisedBy == 'wp_support_plus_supervisor') && $current_user->has_cap('manage_support_plus_agent') && $arrayVisible['raisedby']) ) {
                ?><button class="btn btn-primary wpsp_ticket_nav_btn" onclick="getRaisedByTicketUser(<?php echo $ticket_id; ?>);"><?php _e('Change Raised By', 'wp-support-plus-responsive-ticket-system'); ?></button>&nbsp;<?php
            }
        }
    }
    ...
    <?php if ($current_user->has_cap('manage_support_plus_agent') && $arrayVisible['deleteticket']) { ?>
                <button class="btn btn-danger wpsp_ticket_nav_btn" onclick="wpsp_deleteTicketConfirm(event,<?php echo $ticket_id;?>);"><?php _e($advancedSettings['ticket_label_alice'][10], 'wp-support-plus-responsive-ticket-system'); ?></button>
    <?php }
    ...
    do_action('wpsp_getindivisual_backend_actions_append_inside_more',$ticket);
    ?>        
</div>
<br>
<?php 
...
$ticketActive=true;
if($ticket->active==0){
    $ticketActive=apply_filters('wpsp_show_reply_form_if_inactive',false,$ticket);
}
if ($advancedSettings['wpsp_reply_form_position'] == 1 && apply_filters('wpsp_show_reply_form',true,$ticket) && $ticketActive) {
    ...
    if ($thread->note == 0 || ($thread->note == 1 && $current_user->has_cap('manage_support_plus_ticket')) || ($thread->note == 2 && $current_user->has_cap('manage_support_plus_ticket')) ||($thread->note == 3 && $current_user->has_cap('manage_support_plus_ticket'))||($thread->note == 4 && $current_user->has_cap('manage_support_plus_ticket'))||($thread->note == 5 && $current_user->has_cap('manage_support_plus_ticket'))|| ($thread->note==6 && $current_user->has_cap('manage_support_plus_ticket'))) {
        ?>
            ...
            <?php echo get_avatar($user_email, 60); ?>
                        ...
                        <?php if ($current_user->has_cap('manage_support_plus_agent') && $ticket->active==1) { ?>
                            <div class="wpsp_edit_ticket_threads">
                                <img alt="Edit" style="cursor:pointer;"title="Edit" src="<?php echo WCE_PLUGIN_URL . 'asset/images/edit.png'; ?>"  onclick="edit_thread(<?php echo $thread->id ?>)" />
                            </div>
            <?php
            }
            /* BEGIN CLOUGH I.T. SOLUTIONS MODIFICATION
        ...
        foreach ($canned as $can) {
            ?>
                <tr id="mytr" onclick="replyonclick(<?php echo $can->id; ?>)">
                    ...
                    <td style="display:none;" id="reply<?php echo $can->id; ?>"><?php echo stripcslashes(htmlspecialchars_decode($can->reply,ENT_QUOTES)); ?></td>
...
<script>
    ...
    function replyonclick(cid) {
        ...
        CKEDITOR.instances['replyBody'].insertHtml(x.innerHTML);
        jQuery('#psmwpsp').hide();
...
<?php do_action('wpsp_indivisualticket_backend_button_confirm_messages',$ticket);?>