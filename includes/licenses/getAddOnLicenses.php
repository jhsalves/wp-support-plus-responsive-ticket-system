...
<table id="wpsp_addon_license_tbl">
    ...
    if(class_exists('WPSP_GOOGLE_CAL_EVENT')){
        $license_key=get_option('wpsp_license_key_gcal');
        ?>
        <tr>
            <td><?php _e('Google Calender Events','wp-support-plus-responsive-ticket-system');?></td>
            <td>
                <input type="text" id="wpsp_lisense_txt_gcal" class="wpsp_lisense_txt" value="<?php echo $license_key;?>">
                <?php if(!$license_key){?>
                        <button onclick="wpsp_act_license('gcal',3566)"><?php _e('Save & Activate','wp-support-plus-responsive-ticket-system');?></button>
                <?php } else {?>
                        <button onclick="wpsp_dact_license('gcal',3566,'<?php echo $license_key;?>')"><?php _e('Deactivate','wp-support-plus-responsive-ticket-system');?></button>
                <?php }?>
                <img id="wpsp_license_ajax_loading_gcal" class="wpsp_license_ajax_loading" src="<?php echo WCE_PLUGIN_URL.'asset/images/loading_small.gif';?>"/><br>
                <div id="wpsp_license_error_div_gcal" class="wpsp_license_error_div"></div>
            </td>
            <td>
                <?php if(!$license_key){?>
                        <?php _e('Inactive','wp-support-plus-responsive-ticket-system');?>
                <?php } else {?>
                        <script type="text/javascript">
                            wpsp_check_license_status.push(['gcal',3566,'<?php echo $license_key;?>']);
                        </script>
                        <div id="wpsp_lic_status_gcal" class="wpsp_lic_status"></div>
                        <img id="wpsp_lic_status_ajax_img_gcal" class="wpsp_lic_status_ajax_img" src="<?php echo WCE_PLUGIN_URL.'asset/images/loading_small.gif';?>"/>
                <?php }?>
            </td>
        </tr>
        <?php
    ...
    if(class_exists('WPSP_CANNED_REPLY')){
        $license_key=get_option('wpsp_license_key_acan');
        ?>
        <tr>
            <td><?php _e('Advanced Canned Reply','wp-support-plus-responsive-ticket-system');?></td>
            <td>
                <input type="text" id="wpsp_lisense_txt_acan" class="wpsp_lisense_txt" value="<?php echo $license_key;?>">
                <?php if(!$license_key){?>
                        <button onclick="wpsp_act_license('acan',3569)"><?php _e('Save & Activate','wp-support-plus-responsive-ticket-system');?></button>
                <?php } else {?>
                        <button onclick="wpsp_dact_license('acan',3569,'<?php echo $license_key;?>')"><?php _e('Deactivate','wp-support-plus-responsive-ticket-system');?></button>
                <?php }?>
                <img id="wpsp_license_ajax_loading_acan" class="wpsp_license_ajax_loading" src="<?php echo WCE_PLUGIN_URL.'asset/images/loading_small.gif';?>"/><br>
                <div id="wpsp_license_error_div_acan" class="wpsp_license_error_div"></div>
            </td>
            <td>
                <?php if(!$license_key){?>
                        <?php _e('Inactive','wp-support-plus-responsive-ticket-system');?>
                <?php } else {?>
                        <script type="text/javascript">
                            wpsp_check_license_status.push(['acan',3569,'<?php echo $license_key;?>']);
                        </script>
                        <div id="wpsp_lic_status_acan" class="wpsp_lic_status"></div>
                        <img id="wpsp_lic_status_ajax_img_acan" class="wpsp_lic_status_ajax_img" src="<?php echo WCE_PLUGIN_URL.'asset/images/loading_small.gif';?>"/>
                <?php }?>
            </td>
        </tr>
        <?php
...
<input type="hidden" id="wpsp_license_check_nounce" value="<?php echo wp_create_nonce();?>">
<script type="text/javascript">
