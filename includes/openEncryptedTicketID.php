...
if(isset($_REQUEST['auth'])){
    $auth = sanitize_key($_REQUEST['auth']);
}

$wpsp_hash=new WPSP_Hash_Auth();
if( $isTicketLink && $wpsp_hash->checkAuth($ticket_id, $auth) ){

    /*
     * get ticket values
     */
    $sql = "select subject,type,status,cat_id,priority,active,created_by,guest_name,guest_email
     ...
     */
    ?>
    <h3><?php echo '[' . __($advancedSettings['ticket_label_alice'][1], 'wp-support-plus-responsive-ticket-system') . ' #' . $ticket_id . '] ' . stripcslashes(htmlspecialchars_decode($ticket->subject, ENT_QUOTES)); ?></h3><br>
    <?php
    /*
    ...
    if ($advancedSettings['hide_selected_status_ticket'] != $ticket->status && $ticket->active==1) {
        ...
        ?>
            ...
            if ($thread->note == 0) {
                    ...
                    $body.=$signature;
                    if (!$FrontEndDisplaySettings['wpsp_hideEmail'])
                        $user_email = '';
                    if (!$FrontEndDisplaySettings['wpsp_hideDaysMonthsYearAgo'])
                        $modified = '';
                    if (!$FrontEndDisplaySettings['wpsp_hideExactDate'])
                        $modified_exact_date = '';
                    if (!$FrontEndDisplaySettings['wpsp_hideExactTime'])
                        $modified_exact_time = '';
                ?>
                    ...
                    <?php if (count($attachments)) { ?>
                        <div class="threadAttachment">
                            ...
                            foreach ($attachments as $attachment) {
                                ...
                                $attachCount++;?>
                                <a class="attachment_link" title="Download" target="_blank" href="<?php echo $attach->fileurl; ?>" ><?php echo ($attachCount > 1) ? ', ' : '';?>
            ...
            do_action('wpsp_threads_in_openencryptedticketid',$thread);
        }
    }
    if ($advancedSettings['enable_accordion']) {
        ?>
...
<?php
    ...
    if($isTicketLink){?>
        <div class="wpsp_open_ticket_error">
            <?php _e('This link is expired. Please insert Ticket ID and your Email Address used to create the ticket. New link will be sent to your email.','wp-support-plus-responsive-ticket-system');?>
        </div>
    <?php }
    
    include( WCE_PLUGIN_DIR.'includes/admin/open_ticket/open_ticket_form.php' );
}